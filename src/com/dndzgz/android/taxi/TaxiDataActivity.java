...
public class TaxiDataActivity extends Activity {

	private ProgressDialog m_ProgressDialog = null;
	private Runnable runnableTaxi;
	private JSONObject taxiJson;
...
//	private JSONArray items;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (getIntent().hasExtra("object")) {
			taxiInfo = getIntent().getStringExtra("object");			
		}
		try {
			taxiJson = new JSONObject(taxiInfo);
			taxiJson.put("type", "taxi");
			dndzgzApp.setLastObject(taxiJson);
		...
		String uri = "";
		try {
			uri = "google.navigation:q=" + taxiJson.getString("lat") + "," + taxiJson.getString("lon");			
		...
		Intent FavIntent = new Intent(TaxiDataActivity.this, FavoritesActivity.class);
		...
		actionBar.setTitle(R.string.taxi);
		// ////////////////
		...
		Thread thread = new Thread(null, runnableTaxi, "ObtenerInfoTaxi");
		thread.start();

		m_ProgressDialog = ProgressDialog.show(TaxiDataActivity.this,
				getText(R.string.espere), getText(R.string.obteniendo_datos), true);

	}

	private void getInfoTaxi() {
	...
	private Runnable returnRes = new Runnable() {
		...
		public void run() {
			...
			try {
				titulo.setText(taxiJson.getString("title"));
			} catch (JSONException e) {
			...
			try {
				subtitulo.setText(taxiJson.getString("subtitle"));
			} catch (JSONException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
