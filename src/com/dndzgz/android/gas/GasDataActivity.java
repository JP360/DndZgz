...
public class GasDataActivity extends Activity {

	private ProgressDialog m_ProgressDialog = null;
	private Runnable runnableGas;
	private JSONObject gasJson;
...
//	private JSONArray items;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (getIntent().hasExtra("object")) {
			gasInfo = getIntent().getStringExtra("object");			
		}
		try {
			gasJson = new JSONObject(gasInfo);
			gasJson.put("type", "gas");
			dndzgzApp.setLastObject(gasJson);
		...
		String uri = "";
		try {
			uri = "google.navigation:q=" + gasJson.getString("lat") + "," + gasJson.getString("lon");			
		...
		Intent FavIntent = new Intent(GasDataActivity.this, FavoritesActivity.class);
		...
		actionBar.setTitle(R.string.gasolinera);
		// ////////////////
		...
		Thread thread = new Thread(null, runnableGas, "ObtenerInfoGas");
		thread.start();

		m_ProgressDialog = ProgressDialog.show(GasDataActivity.this,
				getText(R.string.espere), getText(R.string.obteniendo_datos), true);

	}

	private void getInfoGas() {
	...
	private Runnable returnRes = new Runnable() {
		...
		public void run() {
			...
			try {
				titulo.setText(gasJson.getString("title"));
			} catch (JSONException e) {
			...
			try {
				subtitulo.setText(gasJson.getString("subtitle"));
			} catch (JSONException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
